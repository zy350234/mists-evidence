...
/** 逐段编码路径，保留斜杠（对整个路径 encodeURIComponent 会把 / 变成 %2F，导致文件名里出现字面 %2F） */
...
for (const f of files) {
  ...
  let sha;
  const cur = await api("GET", `/repos/${owner}/${repoName}/contents/${encodePath(path)}` + (first ? "" : `?ref=${branch}`));
  ...
  let put = await api("PUT", `/repos/${owner}/${repoName}/contents/${encodePath(path)}`, body);
  if (!put.ok && put.status === 422 && !first) {
    // 空仓库首次提交时不需要指定分支
    delete body.branch;
    put = await api("PUT", `/repos/${owner}/${repoName}/contents/${encodePath(path)}`, body);
  }
  if (!put.ok) {
